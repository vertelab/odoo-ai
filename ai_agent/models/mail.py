...
class MailChannel(models.Model):
    ...
    def message_post(self, **kwargs):
        ...
        if self.channel_type == "chat":
            ai_quest = self.env['res.users'].browse(
                self.channel_member_ids.mapped('partner_id.user_ids.id')
            ).mapped('ai_quest_id')
        ...
        if message.author_id != user.partner_id:
            if ai_quest and self._continue_with_chat(ai_quest, message):  # use the AI as in logged user
