...
class AIQuestoriginal(models.Model):
    _inherit = 'ai.quest'

    ai_agent = fields.Many2many(comodel_name='ai.agent',string='AI Agent',help="") # relation|column1|column2

    def set_agent(self):
        if not self.ai_agent:
            self.ai_agent = self.env["ai.agent"].search([("ai_type", "=", self.ai_type)], limit=1)
...
class AIQuest(models.Model):
    ...
    ai_type = fields.Selection(selection_add=[('perplex', 'Perplexity')])
    
    def chat(self,message):
        _logger.error(f"{session.session=}")
        if self.ai_type == "perplex":
            self.set_agent()
            # Short term memory
            # Context
            # RAG
            # Tools ink searchnx
            response = self.ai_agent.prompt_agent(parser=parser, mail=mail,session=session)
            # save message / response in short term memory
        else:
            return super(AIQuest, self).chat(message)
