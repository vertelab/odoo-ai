...
import time
import yfinance as yf #Need this?
import json
import logging
...
# TODO make this file suitable for langchain
#----------------------------------------------------------------------------------------------------------------------------------------------
...
class LangChainThread(models.TransientModel):
    _inherit = 'openai.thread'

    def thread_values(self, channel, recipient, author):
        return super(LangChainThread, self).thread_values(channel, recipient, author)
    ...
    def openai_client_init(self, user):
        try:
            ...
            _logger.warning(f"LangChain: The server could not be reached {e.__cause__}")
            self.log(f"{e.response}", user.partner_id, role='system')
        except openai.APIStatusError as e:
            self.log(f"LangChain: Status error {e.status_code} {e.response}", user.partner_id, role='system')
            _logger.warning(f"LangChain: Status error {e.status_code} {e.response}")
    ...
    def openai_add_message(self, client, message, role='user'):
        ...
        try:
            ...
            _logger.warning(f"LangChain: Thread The server could not be reached {e.__cause__}")
            self.log(f"LangChain: Thread The server could not be reached {e.__cause__}", status_code=e.status_code,
                     role='openai')
        except openai.RateLimitError as e:
            _logger.warning(f"LangChain: Thread Ratelimit {e.status_code} {e.response}")
            self.log(f"LangChain: Thread Ratelimit {e.status_code} {e.response}", status_code=e.status_code, role='openai')
        except openai.APIStatusError as e:
            _logger.warning(f"LangChain: Thread Status error {e.status_code} {e.response}")
            self.log(f"LangChain: Thread Status error {e.status_code} {e.response}", status_code=e.status_code,
                     role='openai', )
    ...
    def openai_wait4response(self, client):
        ...
        if self.run:
            ...
            if run_status.status == 'expired':
                self.run = None
                self.log(f"LanghChain: Status error run expired {self.run=} {run_status.status=}",
        ...
        if not self.run:
            try:
                ...
                _logger.warning(f"LangChain: Run The server could not be reached {e.__cause__}")
                self.log(f"LangChain: Run The server could not be reached {e.__cause__}", self.recipient_id.parent_id,
                         status_code=e.status_code, role='openai')
                return []
            except openai.RateLimitError as e:
                _logger.warning(f"LangChain: Run Ratelimit {e.status_code} {e.response}")
                self.log(f"LangChain: Run Ratelimit {e.status_code} {e.response}", self.recipient_id.parent_id,
                         status_code=e.status_code, role='openai')
                return []
            except openai.APIStatusError as e:
                _logger.warning(f"LangChain: Run Status error {e.status_code} {e.response}")
                self.log(f"LangChain: Run Status error {e.status_code} {e.response}", self.recipient_id.parent_id,
                         status_code=e.status_code, role='openai', )
                return [{'role': 'assistant', 'content': f"LangChain: Status error {e.status_code} {e.response}"}]
